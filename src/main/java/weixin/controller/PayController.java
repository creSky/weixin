...
public class PayController {
     ...
     *
     * @param wxPrePayDomain * @param request
     *                       * @return
     */
    ...
    public void notifyWeiXinPay(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        logger.info(new Date() + "========================微信支付回调=========================");
        // 从request获取回调参数 转成map
        TreeMap<String, String> params = PayCommonUtil.GetMapFormRequest(request);

        logger.info("用户为=========" + params.get("out_trade_no").split("_")[0]);
        // 校验
        boolean checkFlag = checkOrderStatus(params);

        logger.info("用户" + params.get("out_trade_no").split("_")[0] + "的校验结果为=========" + checkFlag);
    ...
    private synchronized boolean dealWxBill(TreeMap<String, String> params) {
        ...
        logger.info(new Date() + "用户" + params.get("out_trade_no").split("_")[0] + "=====开始处理营销入账=======");
        ...
        logger.info(new Date() + "用户" + params.get("out_trade_no").split("_")[0] + "营销入账参数为=======" + param);
        String returnFlag = JsapiTicketUtil.httpPost(url + param);
        logger.info(new Date() + "用户" + params.get("out_trade_no").split("_")[0] + "营销入账结果为=======" + returnFlag);
